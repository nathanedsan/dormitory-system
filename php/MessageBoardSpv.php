                        ...
                        <div class="container">
                          <form action="MessageBoardMethod.php" method="POST">
                            <div class="input-group">
                              ...
                              <input type="hidden" name="who" value="spv">
...
<script>
      ...
      for (var j of reply_arr) {
        if (j[4] == i[3]) {
          ...
          if (j[5] == spv_id) { // 自己的回覆顯示修改+刪除
            ...
            str += `<button id="delete" class="button-link" value="${del_id}"> 刪除</button>`;
  ...
  function update_msg(page, Mid, Did, No) {
    ...
    original_str += `<input type="hidden" name="who" value="spv">`;
  ...
  function update_reply(page, Rid, Did, No) {
    ...
    while ($row = mysqli_fetch_array($reply_result)) {
      $reply = htmlspecialchars($row[1], ENT_QUOTES);
      echo "arr.push(['$reply', '$row[3]', '$row[0]']);"; // [回覆,日期,replyNo]
    }
    ?>
    // 抓出對應的回覆
    var reply = [];
    for (var i of arr)
      if (i[2] == No)
        reply.push(i[0], i[1], i[2]); // [回覆,日期,replyNo]
