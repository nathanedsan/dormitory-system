...
<body>
                      ...
                      <div class="d-flex no-block align-items-center">
                        ...
                        <form method="post" action="">
                          ...
                          if ((isset($_POST['table1']))) {
                            $_SESSION['selected_table2'] = 'table1';
                            $sql = "SELECT * FROM `apply_record` WHERE `AR_Type`='3' and `AR_Status`='受理中'";
                          ...
                          ?>
                                ...
                                <tbody>
                                  ...
                                  if (mysqli_num_rows($result) > 0) {
                                    foreach ($result as $row) {
                                      ...
                                      $stdresult = mysqli_query($conn, $sql);
                                      $room = mysqli_fetch_assoc($stdresult)['Dorm_No']; //原房間號碼

                                      $stdresult = mysqli_query($conn, $sql);
                                      $building = mysqli_fetch_assoc($stdresult)['Dorm_BuildingName']; //原大樓
                                  ...
                                  ?>
                                      <tr class="table-secondary">
                                        ...
                                        <td><?php echo $buildingch; ?></td>
                                        <td><?php echo $row["AR_Building"]; ?></td>
                                        <td style="display: flex; flex-direction: column; align-items: center;">
                                          <button class="btn btn-common mb-3" style="margin:5px;" onclick='confirm("<?php echo $row["AR_No"]; ?>","1","<?php echo $room; ?>","<?php echo $building; ?>","<?php echo $row["AR_Building"]; ?>","<?php echo $buildingch; ?>")'>核准</button>
                                          <button class="btn btn-secondary mb-3" onclick='confirm("<?php echo $row["AR_No"]; ?>","2","<?php echo $room; ?>","<?php echo $building; ?>","<?php echo $row["AR_Building"]; ?>","<?php echo $buildingch; ?>")'>不核准</button>
                                        </td>
                                      </tr>
                              ...
                              echo '<button class="btn btn-light mb-3" onclick="location.href=\'?page=' . $pages . '\'">末頁</button></div>';
                            ...
                            $_SESSION['selected_table2'] = 'table2';
                            $sql = "SELECT * FROM `apply_record` WHERE `AR_Type`='3' and `AR_Status`='受理完成'";
                            $result = mysqli_query($conn, $sql);
                            $data_nums = mysqli_num_rows($result); // 統計總筆數
                            $per = 7; // 每頁顯示項目數量
                            $pages = ceil($data_nums / $per);
                          ...
                          } elseif (isset($_SESSION['selected_table2'])) {
                            $selectedTable = $_SESSION['selected_table2'];
                          ...
                          if ((isset($_SESSION['selected_table2']))) {
                            $op = $_SESSION['selected_table2'];
                            if ($op == 'table1') {
                              echo '<script>';
  ...
  <script>
    function confirm(Noid, type, room, building, arbuilding, buildingch) {
      Swal.fire({
        title: '確認動作',
        html: '原大樓: ' + buildingch + '<br>原房號: ' + room + '<br>欲更換房號: ' + arbuilding,
        showCancelButton: true,
        confirmButtonText: '確定',
        cancelButtonText: '取消',
      }).then((result) => {
        if (result.isConfirmed) {
          process('adminchangeDor.php', Noid, type, arbuilding, building);
    ...
    function process(url, AR_No, type, arbuilding, building) {
      ...
      optionInput.name = "AR_No";
      optionInput.value = AR_No;
      ...
      optionInput3.type = "hidden";
      optionInput3.name = "arbuilding";
      optionInput3.value = arbuilding;

      const optionInput4 = document.createElement("input");
      optionInput4.type = "hidden";
      optionInput4.name = "building";
      optionInput4.value = building;

      form.appendChild(optionInput);
      form.appendChild(optionInput2);
      form.appendChild(optionInput3);
      form.appendChild(optionInput4);
